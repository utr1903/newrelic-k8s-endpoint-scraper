...
func parseConfigFile() (
	*Config,
	...
	if configPath == "" {
		fmt.Println(CONFIG__CONFIG_PATH_IS_NOT_DEFINED)
		return nil, errors.New(CONFIG__CONFIG_PATH_IS_NOT_DEFINED)
	}

	// Read config file
	configFile, err := readFile(configPath)
	if err != nil {
		fmt.Println(CONFIG__CONFIG_FILE_COULD_NOT_BE_READ)
		return nil, errors.New(CONFIG__CONFIG_FILE_COULD_NOT_BE_READ)
	}
	...
	if err != nil {
		fmt.Println(CONFIG__CONFIG_FILE_COULD_NOT_BE_PARSED_INTO_YAML)
		return nil, errors.New(CONFIG__CONFIG_FILE_COULD_NOT_BE_PARSED_INTO_YAML)
	}
...
func parseNewRelicLicenseKey() (
	...
	if nrLicenseKey == "" {
		fmt.Println(CONFIG__LICENSE_KEY_IS_NOT_PROVIDED)
		return "", errors.New(CONFIG__LICENSE_KEY_IS_NOT_PROVIDED)
	}
...
func setNewRelicEventsEndpoint(
	...
	if nrAccountId == "" {
		fmt.Println(CONFIG__ACCOUNT_ID_IS_NOT_PROVIDED)
		return "", errors.New(CONFIG__ACCOUNT_ID_IS_NOT_PROVIDED)
	}
...
func checkEndpoints(
	...
	if cfg.Endpoints == nil || len(cfg.Endpoints) == 0 {
		cfg.Logger.Log(logrus.ErrorLevel, CONFIG__NO_ENDPOINT_IS_DEFINED)
		return errors.New(CONFIG__NO_ENDPOINT_IS_DEFINED)
	}

	for _, endpoint := range cfg.Endpoints {
		if endpoint.Type == "" || endpoint.Name == "" || endpoint.URL == "" {
			cfg.Logger.Log(logrus.ErrorLevel, CONFIG__ENDPOINT_INFO_IS_MISSING)
			return errors.New(CONFIG__ENDPOINT_INFO_IS_MISSING)
		}

		if endpoint.Type != "kvp" {
			cfg.Logger.Log(logrus.ErrorLevel, CONFIG__ENDPOINT_TYPE_IS_NOT_SUPPORTED)
			return errors.New(CONFIG__ENDPOINT_TYPE_IS_NOT_SUPPORTED)
		}
	}
